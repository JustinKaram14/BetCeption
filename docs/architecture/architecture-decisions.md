...
| --- | --- | --- |
| Zuverlässigkeit | ACID-Transaktionen, pessimistic locking, Idempotenz | `AppDataSource.transaction` in Round/Powerup/Wallet, Sperren auf `User`- und `UserPowerup`-Tabellen; API-Design erzwingt maximal eine aktive Runde pro User. |
