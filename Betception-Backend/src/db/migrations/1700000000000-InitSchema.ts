...
export class InitSchema1700000000000 implements MigrationInterface {
    ...
    await queryRunner.query(`
      INSERT INTO sidebet_types (code, title, description, base_odds, config_json) VALUES
        (
          'FIRST_CARD_COLOR',
          'First Card Color',
          'Guess whether the first revealed card is red or black',
          2.000,
          JSON_OBJECT(
            'fields', JSON_ARRAY('predicted_color'),
            'context', JSON_ARRAY('FIRST_PLAYER_CARD','FIRST_DEALER_CARD')
          )
        ),
        (
          'FIRST_CARD_SUIT',
          'First Card Suit',
          'Guess the suit of the first revealed card',
          4.000,
          JSON_OBJECT(
            'fields', JSON_ARRAY('predicted_suit'),
            'context', JSON_ARRAY('FIRST_PLAYER_CARD','FIRST_DEALER_CARD')
          )
        ),
        (
          'FIRST_CARD_RANK',
          'First Card Rank',
          'Guess the rank of the first revealed card',
          13.000,
          JSON_OBJECT(
            'fields', JSON_ARRAY('predicted_rank'),
            'context', JSON_ARRAY('FIRST_PLAYER_CARD','FIRST_DEALER_CARD')
          )
        )
    ...
    await queryRunner.query(`
      INSERT INTO powerup_types (code, title, description, min_level, price, effect_json) VALUES
        (
          'MULTI_PLUS',
          'Multiplier Pill',
          'Increase sidebet multipliers by 20%',
          3,
          500,
        ...
        (
          'JOKER_CARD',
          'Joker Pill',
          'Single-use shield: convert a bust into a push',
          5,
          1000,
          JSON_OBJECT('joker', 1)
        ),
        (
          'NO_LOSS',
          'Protection Pill',
          '10% chance to refund a losing bet',
          7,
          1500,
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP VIEW IF EXISTS leaderboard_level;`);
    await queryRunner.query(`DROP VIEW IF EXISTS leaderboard_weekly_winnings;`);
    await queryRunner.query(`DROP VIEW IF EXISTS leaderboard_balance;`);

    await queryRunner.query(`DROP TABLE IF EXISTS powerup_consumptions;`);
    await queryRunner.query(`DROP TABLE IF EXISTS user_powerups;`);
    await queryRunner.query(`DROP TABLE IF EXISTS powerup_types;`);
    await queryRunner.query(`DROP TABLE IF EXISTS daily_reward_claims;`);
    await queryRunner.query(`DROP TABLE IF EXISTS wallet_transactions;`);
    await queryRunner.query(`DROP TABLE IF EXISTS side_bets;`);
    await queryRunner.query(`DROP TABLE IF EXISTS sidebet_types;`);
    await queryRunner.query(`DROP TABLE IF EXISTS main_bets;`);
